...
class TLSTM(object):
    ...
    def __init__(self, input_dim, output_dim, hidden_dim, fc_dim,train):
        ...
        self.input = tf.placeholder(dtype = tf.float32, shape=[None, None, self.input_dim])
        self.labels = tf.placeholder(dtype = tf.float32, shape=[None, output_dim])
        self.time = tf.placeholder(dtype = tf.float32, shape=[None, None])
